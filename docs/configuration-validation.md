...
Address reachability and free ports are runtime checks. HTTP authentication and
HTTP/RPC/worker-peer TLS are active with independent configuration boundaries;
see [runtime TLS](runtime-tls.md). Write-queue fields include settings not wired
into the runtime.
