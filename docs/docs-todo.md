...
- **Security acceptance remains open.** This stack implements HTTPS, API roles,
  RPC/data/replica mTLS and secret handling. Complete the original
  [WIP-17 acceptance audit](trds/WIP-17/completion.md) before claiming the entire
  security model is complete. Metrics use a separate public listener.
- WIP-12 named late streams and durable windows are implemented on this stack;
  preserve their [runtime limits](trds/WIP-12/runtime-contract.md) when updating
  general guides. Error-policy DLQs are a separate feature.
...
Pause/resume implement savepoint-backed suspension and restore on this stack;
see the [lifecycle guide](job-cli.md) for asynchronous completion and failure behavior.
