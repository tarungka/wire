...
Without a formal RPC specification, it is impossible to implement the Coordinator or Worker, write integration tests, reason about failure modes, or build compatible tooling. Every other TRD (WIP-01 SDK submission, WIP-02 connector lifecycle, WIP-04 job lifecycle, WIP-08 two-phase commit) depends on well-defined RPC semantics that do not yet exist.
...
| -- | -- |
| Define all Coordinator-Worker RPCs with full message types | Worker-to-Worker data plane protocol (see WIP-06) |
| Specify error codes and failure semantics for each RPC | REST API for external clients (see WIP-04) |
| Document transport layer (Yamux streams, framing, serialization) | Authentication/authorization token format (see WIP-09) |
| Define Heartbeat protocol with load reporting and command dispatch | Coordinator HA leader election protocol (see WIP-07) |
...
1. REST API handler receives job submission (see WIP-04).
...
message OperatorDescriptor {
  ...
  string operator_name = 2;                  // Human-readable name (e.g., "kafka-source")
...
* **Client Authentication:** Workers present their certificate when connecting to the Coordinator. The Coordinator validates the certificate chain against the CA. Enabled via `--node-verify-client` flag (see WIP-03).
...
* RPC method authorization is deferred to WIP-09 (Security Model).
...
| 3 | How does the RPC layer handle Coordinator leader failover? Workers currently connect to a single Coordinator address. Should Workers discover the new leader via a redirect response, or should they re-resolve via DNS/service discovery? See WIP-07. | Tarun | Open |
