...
| 8 | Barrier ordering | T `TestBarrierOrdering` and exact 100-stream mixed-message comparisons; E output-router ordering tests cover each output partition; `TestAlignedBarrierPreservesQueuedAndBufferedRecordOrder` verifies snapshot contents and pre/barrier/post output order. |
...
| 14 | TLS transparency | Valid-message, 100-stream, failure-threshold/reset, task rejection and crossed-connection retirement tests run with mutual TLS. `TestTransportFrameBoundaries`, `TestPartialDataFrameDeadline`, `TestDataWindowWriteTimeout`, negotiation and first-frame rejection tests also run over TCP and mutual TLS. |
...
| 18 | Compatible versions/data | T `TestSessionNegotiationVersionsAndFeatures/{tcp,mutual_tls}/same_version` checks inherited parameters and actual record/EOP flow. |
| 19 | Incompatible versions | T `TestSessionNegotiationVersionsAndFeatures/{tcp,mutual_tls}/incompatible` checks both errors and session closure; 1,000 race runs cover the reply/close race. |
