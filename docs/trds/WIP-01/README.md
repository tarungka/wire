...
sources:
  - name: "source-name"
    type: "kafka"                    # Connector type (see WIP-02)
    parallelism: 8
    watermark:
...
* **State storage:** All keyed state is stored in Pebble, scoped to `(KeyGroup, OperatorID, UserKey)`. See WIP-12 for key encoding.
* **Checkpoint storage:** State snapshots are uploaded to S3/MinIO or local filesystem. See `state-backend.md` Canon doc.
* **Job metadata:** Job graph, configuration, and checkpoint history stored in Coordinator's metadata store (see WIP-07).
...
| **Rationale** | Zero-copy through the pipeline. No serialization overhead between operators in the same chain. User controls serialization format. Matches Kafka semantics. |
...
| -- | -- | -- | -- | -- |
| 1 | User Map function panics | Runtime catches panic via `recover()`, wraps as error, routes event to DLQ (see WIP-13) | Single event lost | Medium |
...
* SDK communicates with Coordinator via authenticated RPC (see WIP-09 for auth model).
