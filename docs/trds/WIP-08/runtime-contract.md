...
The coordinator checks health at most every 250ms (more frequently for short configured timeouts), independently of scheduling or deployment RPCs. This fast path only marks expired workers LOST and removes their available capacity in memory, then wakes the scheduler. Assignment reads and durable task/job recovery run on the scheduler path, under the ownership lock to prevent stale loss publication against a replacement attempt. A heartbeat arriving after expiry is refused even if the periodic detector has not run. Expired workers cannot refresh their authority with an old heartbeat; they must re-register. Heartbeats on an obsolete registration session are rejected, including legacy workers. Epoch checks precede liveness/capacity updates and command draining.
