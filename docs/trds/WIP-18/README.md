...
- **SDK selection:** `env.SetStateBackend(sdk.NewHashMapStateBackend(maxMemoryMB))` or `env.SetStateBackend(sdk.NewPebbleStateBackend(dataDir))` configures embedded Process instances. The default is temporary Pebble state, cleaned on operator close; an explicit directory retains state under operator/instance subdirectories. Reusing that directory requires the same graph node IDs and parallelism; it is not savepoint rescaling or coordinated source replay. Explicit cluster selection is rejected until integration exists. Storage errors fail the Process invocation before its results are emitted.
- **Pebble lifecycle:** `PebbleDataDir` contains an `ACTIVE` generation pointer and checkpoint directories. Checkpoint handles reference local files with SHA-256 hashes; they do not upload files or provide remote durability. Restore validates a private copy before switching generations; restore and close invalidate outstanding iterators. Successful restore removes the old database generation, but checkpoint retention and abandoned-directory cleanup are not automated. Do not edit native checkpoint files: SSTs can be hard-linked to live data.
- **Evidence:** [state_backend_hashmap.go](../../../internal/engine/state_backend_hashmap.go), [state_backend_factory.go](../../../internal/engine/state_backend_factory.go), [state_backend_pebble.go](../../../internal/engine/state_backend_pebble.go), [state_backend_pebble_test.go](../../../internal/engine/state_backend_pebble_test.go).
