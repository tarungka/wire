...
- **Remaining:** Automatic cluster offset restore/replay orchestration, SDK transactional connector integration, automatic runtime batching/DLQ routing, the developer trial, and broader SDK/cluster validation remain incomplete. HTTP acknowledgements are volatile; restoring a sequence does not recover events or trigger sender replay. Worker HTTP source lifecycle depends on WIP-20. Production third-party connectors remain outside this proposal's HTTP-focused scope.
- **Validation:** Connector race tests reached 95.2% statement coverage. Concurrent HTTP ingress verified atomic bounded acceptance and capacity recovery, and a saturation benchmark ran successfully. See [validation results](validation.md) for measured scope and limitations.
