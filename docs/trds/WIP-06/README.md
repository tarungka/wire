...
| -- | -- |
| Define byte-level frame format with length prefix | RPC protocol between Coordinator and Workers (see WIP-05) |
...
| **SourceID** | `"s"` | `str` | Yes | Identifier of the source operator or subtask that generated this watermark. Format: `"{operator_name}-{subtask_index}"` (e.g., `"kafka-source-0"`). Used for debugging and multi-input watermark tracking. |
