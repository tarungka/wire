...
- **Remaining:** YAML policy parsing and reserved `__dlq__` graph input, exported runtime metrics (executors currently use noop counters), full MiniCluster error-policy coverage, and the stated 100% unit-coverage target. Ordinary operator lifecycle fixes are in the separate WIP-20 PR; this change owns the added DLQ sink lifecycle.
