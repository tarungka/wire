...
3. Preserve per-operator policies through SDK graph conversion and worker deployment. Exercise real MiniCluster paths for all five named scenarios: 99 normal/1 DLQ, transient sink retry, permanent sink failure, panic-to-DLQ continuation, and missing-DLQ rejection.
...
5. Preserve best-effort DLQ semantics: failed/full writes log and count drops; missing destinations and failed Open reject execution; successful delivery counts once. Verify sink Open/Close ownership and isolation from normal checkpoint transactions. Document that best-effort DLQ can lose records and can duplicate on replay.
...
records as delivered, and DLQ startup failures being silently converted into dropped records.
