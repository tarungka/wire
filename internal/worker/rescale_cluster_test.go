...
type rescaleStateBackend interface {
	engine.StateBackend
	RestoreKeyGroupRanges(context.Context, keygroup.KeyGroupRange, []engine.KeyGroupSnapshot) error
	VisitKeyGroupRange(context.Context, keygroup.KeyGroupRange, func([]byte, []byte) error) error
}

type rescaleClusterSource struct {
	checkpointTestSource
	backend  rescaleStateBackend
		...
		for _, sizes := range [][2]int{{4, 8}, {8, 4}, {4, 3}, {2, 3}, {1, 2}} {
			t.Run(fmt.Sprintf("%s/%d-to-%d", kind, sizes[0], sizes[1]), func(t *testing.T) {
				...
				registry.RegisterMap("state", func(_ context.Context, _ []byte, tc worker.TaskContext) (engine.MapOperator, error) {
					backend, err := engine.NewStateBackend(engine.StateBackendConfig{Type: kind, PebbleDataDir: t.TempDir()})
					if err != nil {
						return nil, err
					}
					return &rescaleClusterMap{&rescaleClusterSource{backend: backend.(rescaleStateBackend), groups: tc.KeyGroup, restored: &restored}}, nil
				})
