...
func (m *MockDataSink) Write(ctx context.Context, data <-chan *models.Job, initialData <-chan *models.Job) error {
	return m.Called(ctx, data, initialData).Error(0)
}
...
func createJobChannel(t *testing.T, data ...string) chan *models.Job {
	t.Helper()
	jobs := make([]*models.Job, len(data))
	for i, d := range data {
		job, err := models.New(map[string]interface{}{"data": d})
...
func TestNewDataPipeline(t *testing.T) {
	...
	assert.Equal(t, uint(runtime.NumCPU()), pipeline.jobCount)
...
func TestDataPipeline_Run(t *testing.T) {
	...
	initialDataChan := createJobChannel(t)
	dataChan := createJobChannel(t)
	...
	mockSink.On("Write", mock.Anything, mock.Anything, mock.Anything).Return(nil)
...
func TestDataPipeline_Run_SourceConnectError(t *testing.T) {
	...
	mockSink.On("Connect", mock.Anything).Return(nil).Maybe()
	mockSource.On("Info").Return("MockSource").Maybe()
	mockSink.On("Info").Return("MockSink").Maybe()
	mockSource.On("Name").Return("MockSource").Maybe()
	mockSink.On("Name").Return("MockSink").Maybe()
...
func TestDataPipeline_Run_LoadInitialDataError(t *testing.T) {
	...
	mockSource.On("Connect", mock.Anything).Return(nil)
	mockSink.On("Connect", mock.Anything).Return(nil)
	mockSource.On("LoadInitialData", mock.Anything, mock.Anything).Return((<-chan *models.Job)(make(chan *models.Job)), loadErr)
	mockSource.On("Info").Return("MockSource").Maybe()
	mockSink.On("Info").Return("MockSink").Maybe()
	mockSource.On("Name").Return("MockSource").Maybe()
	mockSink.On("Name").Return("MockSink").Maybe()
...
func TestDataPipeline_Run_ReadError(t *testing.T) {
	...
	mockSource.On("Connect", mock.Anything).Return(nil)
	mockSink.On("Connect", mock.Anything).Return(nil)
	mockSource.On("LoadInitialData", mock.Anything, mock.Anything).Return((<-chan *models.Job)(make(chan *models.Job)), nil)
	mockSource.On("Read", mock.Anything, mock.Anything).Return((<-chan *models.Job)(make(chan *models.Job)), readErr)
	mockSource.On("Info").Return("MockSource").Maybe()
	mockSink.On("Info").Return("MockSink").Maybe()
	mockSource.On("Name").Return("MockSource").Maybe()
	mockSink.On("Name").Return("MockSink").Maybe()
...
func TestDataPipeline_DataFlow(t *testing.T) {
	...
	initialJobChan := createJobChannel(t, "initial data 1", "initial data 2")
	jobChan := createJobChannel(t, "data 1", "data 2")
