...
import (
	...
	"github.com/tarungka/wire/pkg/core/checkpoint"
...
type PragmaCheckRequest commandProto.Request
...
// CheckpointStore is the interface Checkpoint stores must implement.
type CheckpointStore interface {
	...
	// Stats returns stats about the Checkpoint Store.
...
type Store struct {
	...
	// Checkpoint
	snapshotDir   string // To be renamed to checkpointDir later if desired
	snapshotStore CheckpointStore // To be renamed to checkpointStore later
...
func (s *Store) Open() (retError error) {
	...
	checkpointStore, err := checkpoint.NewStore(filepath.Join(s.snapshotDir))
	if err != nil {
		return fmt.Errorf("failed to create snapshot store: %s", err)
	}
	checkpointStore.LogReaping = s.hcLogLevel() < hclog.Warn
	s.snapshotStore = checkpointStore // snapshotStore field should probably be renamed to checkpointStore too
...
func (s *Store) Execute(ex *commandProto.ExecuteRequest) ([]*commandProto.ExecuteQueryResponse, error) {
...
type fsmExecuteQueryResponse struct {
	results []*commandProto.ExecuteQueryResponse
	error   error
}
...
func (s *Store) Query(qr *commandProto.QueryRequest) ([]*commandProto.QueryRows, error) {
	if !s.open.Is() {
		return nil, ErrStoreNotOpen
	}
	...
	if qr.Request == nil || len(qr.Request.Statements) == 0 || qr.Request.Statements[0] == nil || qr.Request.Statements[0].Sql == "" {
	...
	var resultRows []*commandProto.QueryRows
	err := s.db.View(func(txn *badger.Txn) error {
		...
		if err != nil {
			if errors.Is(err, badger.ErrKeyNotFound) {
				...
				resultRows = []*commandProto.QueryRows{
					{
						Columns: []string{"key", "value"},
						Types:   []string{"text", "text"},
						Values:  make([]*commandProto.Values, 0), // Empty values
					},
				}
		...
		// Key Parameter
		keyParam := &commandProto.Parameter{Value: &commandProto.Parameter_S{S: queryKey}}
		// Value Parameter - store as bytes (blob)
		valueParam := &commandProto.Parameter{Value: &commandProto.Parameter_Y{Y: valueBytes}}

		rowValues := &commandProto.Values{
			Parameters: []*commandProto.Parameter{keyParam, valueParam},
		}
		...
		resultRows = []*commandProto.QueryRows{
			{
				...
				Values:  []*commandProto.Values{rowValues},
			},
		}
		return nil
	})
...
func (s *Store) Request(eqr *commandProto.ExecuteQueryRequest) ([]*commandProto.ExecuteQueryResponse, error) {
	...
	c := &commandProto.Command{
		Type:       commandProto.Command_COMMAND_TYPE_EXECUTE_QUERY,
		SubCommand: b,
		Compressed: compressed,
	}
...
func (s *Store) load(lr *commandProto.LoadRequest) error {
	...
	c := &commandProto.Command{
		Type:       commandProto.Command_COMMAND_TYPE_LOAD,
		SubCommand: b,
	}
...
func (s *Store) fsmSnapshot() (fSnap raft.FSMSnapshot, retErr error) {
	...
	return checkpoint.NewSnapshot(pipeReader), nil
}
...
func (s *Store) fsmApply(l *raft.Log) (e interface{}) {
	...
	case commandProto.Command_COMMAND_TYPE_EXECUTE:
		...
		// FIXME: Decompression needed if cmd.Compressed is true and command.UnmarshalExecuteRequest is not available.
		// For now, assuming cmd.SubCommand is uncompressed or proto.Unmarshal handles it.
		if err := proto.Unmarshal(cmd.SubCommand, &ex); err != nil { // This 'proto' should be from google.golang.org/protobuf/proto
			s.logger.Error().Err(err).Msg("failed to unmarshal execute request from sub_command")
			return &fsmExecuteQueryResponse{error: fmt.Errorf("unmarshal execute request from sub_command: %w", err)}
		}

		if ex.Request == nil || len(ex.Request.Statements) == 0 {
		...
		case "SET":
			...
			resp.results[0] = &commandProto.ExecuteQueryResponse{
				Result: &commandProto.ExecuteQueryResponse_E{
					E: &commandProto.ExecuteResult{
						LastInsertId: 0, // Not applicable for SET
						RowsAffected: 1,
					},
				},
			}
